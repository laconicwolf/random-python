...
# Adds the assemblies to CLR. CLR only includes a reference to System by default.
...
def main():
    ...
    word_banner = '{} version: {}. Coded by: {}'.format(sys.argv[0], __version__, __author__)
    ...
    print
    print '[*] Guessing {} user(s) and {} password(s). {} total guesses.'.format(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Guess a supplied list of passwords against a supplied list of usernames.'
    )
    parser.add_argument(
        '-u', '--username',
    ...
    parser.add_argument(
        '-d', '--domain',
        help='Specify a domain. Defaults to the local domain of the computer the script is running from.'
    )
    parser.add_argument(
        '-p', '--password',
    ...
    parser.add_argument(
        '-uf', '--username_file',
        help='Specify the path of a file containing usernames to guess password(s) against.'
    )
    parser.add_argument(
        '-pf', '--password_file',
        help='Specify the path of a file containing passwords to guess against username(s).'
    )
    parser.add_argument(
        "-t", "--threads",
        nargs="?",
    ...
    if not args.username and not args.username_file:
        parser.print_help()
        print '\n[-] Please specify either a single username (-u) or the path to a file listing usernames (-uf).'
        exit()
    if not args.password and not args.password_file:
        parser.print_help()
        print '\n[-] Please specify either a single password (-p) or the path to a file listing passwords (-pf).'
        exit()
    if args.username and args.username_file:
        parser.print_help()
        print '\n[-] Please specify either a single username (-u) or the path to a file listing usernames (-uf). Not both.'
        exit()
    if args.password and args.password_file:
        parser.print_help()
        print '\n[-] Please specify either a single password (-p) or the path to a file listing passwords (-pf). Not both.'
        exit()
    ...
    if args.username_file:
        if not os.path.exists(args.username_file):
            print '\n[-] The file {} does not exist or you do not have access to it. Please check the path and try again.'.format(args.username_file)
            exit()
    ...
    if args.password_file:
        if not os.path.exists(args.password_file):
            print '\n[-] The file {} does not exist or you do not have access to it. Please check the path and try again.'.format(args.password_file)
            exit()
