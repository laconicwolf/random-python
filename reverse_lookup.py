...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--range",
                        help="Specify the IP range to test. \
Example: ./{0} -r 192.168.0.0/24 or ./{0} -r 192.168.0.0-255".format(sys.argv[0]))
    ...
    if args.file:
        if not os.path.exists(args.file):
            parser.print_help()
            print(
                "\n[-]  The file cannot be found or you do not have permission to \
                open the file. Please check the path and try again\n")
            exit()
