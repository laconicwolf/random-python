...
if __name__ == '__main__':
	...
	parser.add_argument(
		"-ch", "--custom_header", 
		nargs='*',
	    help=('Specify one or more custom header and value delimited. Example: ',
	    	 '-ch "X-Custom-Header: CustomValue"')
	)
	parser.add_argument(
		"-a", "--auth",
	    help=('Specify a username and password delimited with ":" for basic authentication. ',
	          'Example: -a myuser:mypassword.')
	)
	parser.add_argument(
		"-c", "--cookies",
	    nargs="*",
	    help=('Specify cookie(s). Example: ',
	    	  '-c "C1=IlV0ZXh0L2h; C2=AHWqTUmF8I;"')
	)
	parser.add_argument(
		"-ua", "--useragent", 
	...
	parser.add_argument(
		"-csv", "--csv_name",
	    help=("Specify a name for the csv file. ",
	         "Default is {}_results<timestamp>.csv".format(sys.argv[0][:-3]))
	)
